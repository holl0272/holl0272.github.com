...
'//////////////////////////////////////////////////////////////////
	...
	Public Sub GetUSPSRates(aryCarrierInfo)
		...
		if pstrDestinationCountryAbb = "US" then
			...
			mstrURL = "http://" & cstrServer & "Rate"
			mstrRequestData = "<RateRequest USERID=" & Chr(34) & pstrUsername & Chr(34) & " PASSWORD=" & Chr(34) & pstrPassword & Chr(34) & ">"
			...
			mstrRequestData = mstrRequestData & "<Package ID=" & Chr(34) & "0" & Chr(34) & ">" _
					& "<Service>Express</Service>" _
					...
					& "<Container>" & pstrContainer & "</Container>" _
					& "<Size>" & pstrSize & "</Size>" _
					& "<Machinable></Machinable>" _
					& "</Package>" _
					& "<Package ID=" & Chr(34) & "1" & Chr(34) & ">" _
					& "<Service>Priority</Service>" _
					& "<ZipOrigination>" & pstrOriginZip & "</ZipOrigination>" _
					& "<ZipDestination>" & p_strZip & "</ZipDestination>" _
					& "<Pounds>" & plngPounds & "</Pounds><Ounces>" & plngOunces & "</Ounces>" _
					& "<Container>" & pstrContainer & "</Container>" _
					...
					& "<Machinable></Machinable>" _
					& "</Package>" _
					& "<Package ID=" & Chr(34) & "2" & Chr(34) & ">" _
					& "<Service>Parcel</Service>" _
					& "<ZipOrigination>" & pstrOriginZip & "</ZipOrigination>" _
					& "<ZipDestination>" & p_strZip & "</ZipDestination>" _
					& "<Pounds>" & plngPounds & "</Pounds><Ounces>" & plngOunces & "</Ounces>" _
					& "<Container>" & pstrContainer & "</Container>" _
					& "<Size>" & pstrSize & "</Size>" _
					& "<Machinable>" & pblnMachinable & "</Machinable>" _
					& "</Package>" _
					& "<Package ID=" & Chr(34) & "3" & Chr(34) & ">" _
			...
			If pobjXMLDoc.loadXML(pstrRawData) Then
				If pobjXMLDoc.documentElement.nodeName <> "Error" then 'Top-level Error
					...
					For i =0 To nodeList.length - 1
						...
						For j =0 To n.childNodes.length - 1
							Set e = n.childNodes.Item(j)
							If e.nodeName = "Service" Then pstrKey = e.firstChild.nodeValue
							If e.nodeName = "Postage" Then psngPostage = e.firstChild.nodeValue
						Next 'j
						If (Len(pstrKey) > 0) And (Len(psngPostage) > 0) then
							If Not pobjDic.Exists(pstrKey) Then
								set pclsShippingOption = New clsShippingOption
			...
			mstrURL = "Http://" & cstrServer & "IntlRate"
			mstrRequestData = "<IntlRateRequest USERID=" & Chr(34) & pstrUsername & Chr(34) & " PASSWORD=" & Chr(34) & pstrPassword & Chr(34) & ">" _
							& "<Package ID=" & Chr(34) & "0" & Chr(34) & ">" _
							& "<Pounds>" & plngPounds & "</Pounds><Ounces>" & plngOunces & "</Ounces>" _
							& "<MailType>" & pstrMailType & "</MailType>" _
							& "<Country>" & pstrTempCountry & "</Country>" _
							& "</Package>" _
							& "</IntlRateRequest>"
	...
	Dim pstrKey
		...
		For i =0 To ochild.childNodes.length - 1
			...
			Select Case s.nodeName
				...
				Case "SvcDescription": pstrSvcDescription = s.firstChild.nodeValue
			End Select
		Next
...
'			pclsShippingOption.shipRate = ShipMultiple(pstrProductBasedShippingCode)
	...
	Dim pvntTempShipCode, pstrTempShipMethodName, pvntTempShipping
		...
		Else
			...
			For each vItem in pobjDic
				With pobjDic.Item(vItem)
					If CDbl(.Postage) < CDbl(pvntTempShipping) And (.ShippingCode <> cstrLocalPickupCode) Then
...
Const orderCutoffTime = "12:00:00 PM"
