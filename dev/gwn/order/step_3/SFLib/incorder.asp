...
'--------------------------------------------------------------------
Function getProdAttrID(byVal sPrefix, byVal iOrderID, byRef iProdAttrNum)
...
Dim iArrayBound
	...
	Select Case sPrefix
		Case "odrattrtmp"
			pstrSQL = "SELECT odrattrtmpID FROM sfTmpOrderAttributes WHERE odrattrtmpOrderDetailId = " & makeInputSafe(iOrderID)
		Case "odrattrsvd"	
			pstrSQL = "SELECT odrattrsvdID FROM sfSavedOrderAttributes WHERE odrattrsvdOrderDetailId = " & makeInputSafe(iOrderID)
	End Select

	If vDebug = 1 Then Response.Write "<p> getProdAttrID SQL: " & pstrSQL & "<br /> iProdAttrNum: " & iProdAttrNum
	
	Set rsGetAttr = CreateObject("ADODB.RecordSet")
	rsGetAttr.Open pstrSQL, cnn, adOpenKeyset, adLockOptimistic, adCmdText
...
Dim rsGetIDNames, aIdNames(3), sLocalSQL
	  ...
	  sLocalSQL = "SELECT sfManufacturers.mfgName, sfVendors.vendName, sfCategories.catName FROM sfManufacturers, sfVendors, sfCategories WHERE "_
			 & " sfManufacturers.mfgID = " & makeInputSafe(iProdManufacturerID) & " AND sfVendors.vendID = " & makeInputSafe(sProdVendorID) & " AND sfCategories.catID = " & makeInputSafe(iProdCategoryID)

	  If vDebug = 1 Then Response.Write "<p> getIDName SQL : " & sLocalSQL

      Set rsGetIdNames = CreateObject("ADODB.RecordSet")
...
Dim rsOrderInput, iLocalID
        ...
        Set rsOrderInput = CreateObject("ADODB.RecordSet")
	...
	rsOrderInput.Fields("odrdttmpSessionID")	= SessionID
	rsOrderInput.Fields("odrdttmpHttpReferer")	= sReferer
	rsOrderInput.Update
	iLocalID = rsOrderInput.Fields("odrdttmpID")
	
	If vDebug = 1 Then Response.Write "<p><font color=""#335544"" size=""6""> Tmp Order ID = "  & makeInputSafe(iLocalID) & "</font>"
...
Dim rsAttributeInput, iLocalResult, sLocalSQL
	...
	Set rsAttributeInput = CreateObject("ADODB.RecordSet")
	...
	sLocalSQL = "SELECT odrattrtmpID FROM sfTmpOrderAttributes WHERE odrattrtmpName ='"_
				 & sAttrName & "' AND odrattrtmpOrderDetailID = " & makeInputSafe(iOrderDetailID)
...
Dim rsCopy, sLocalSQL, rsSvdCart, rsSvdCartAttr, iKeyID, sReferer,sDateTime,sTmpAttrName, sTmpAttrID, aTmpOrderArray
	...
	sLocalSQL = "Select * FROM sfTmpOrderDetails INNER JOIN sfTmpOrderAttributes ON odrdttmpID = odrattrtmpOrderDetailId "_
				& " WHERE odrdttmpID = " & makeInputSafe(iTmpOrderDetailID) & " And odrdttmpSessionID = " & SessionID

	If vDebug = 1 Then Response.Write "<p>getCopyToSvdTable SQL : " & sLocalSQL

	Set rsCopy = CreateObject("ADODB.RecordSet")
	...
	Else
		...
		' Copy to svd cart			
		Set rsSvdCart = CreateObject("ADODB.RecordSet")
			...
			rsSvdCart.Open "sfSavedOrderDetails", cnn, adOpenDynamic, adLockOptimistic
				...
				rsSvdCart.Fields("odrdtsvdSessionID")	= SessionID
		...
		Do While Not rsCopy.EOF		
			...
			sTmpAttrID = Trim(rsCopy.Fields("odrattrtmpAttrID"))
				Set rsSvdCartAttr = CreateObject("ADODB.RecordSet")
...
Dim sLocalSQL, aLocalProdArray(3), rsSelectProd
	...
	sLocalSQL = "SELECT prodName, prodNamePlural, prodMessage, prodAttrNum FROM sfProducts WHERE prodEnabledIsActive=1 AND prodID = '"& makeInputSafe(sProdID) & "'"

  	  Set rsSelectProd = CreateObject("ADODB.RecordSet")
