...
Const cblnUsessPricingLevels = True
...
Dim mblnQtyBoxAttributeUsed:	mblnQtyBoxAttributeUsed = False	
...
Function MPOPrefix(byVal strProductID)

Dim pstrTemp

	pstrTemp = cstrSSMPOAttributeDelimiter & Replace(strProductID, " ", "")
	MPOPrefix = pstrTemp

End Function	'MPOPrefix
...
Dim pstrTempID
	...
	Else
		CleanAttributeArray = aryAttributes
...
Sub GetAttributes(byVal strProductID, byRef aProdAttr, byRef iProdAttrNum, byVal lngCounter_ssMPO)
'Purpose: fills aProdAttr with attribute selection from Request.Form
'Returns: Nothing
'		  sets values for aProdAttr which is byRef

ReDim aProdAttr(iProdAttrNum-1)
Dim i
Dim pstrFieldName
Dim pstrNewVar		
	...
	If mblnMPOPage_ssMPO Then
		For i = 1 to iProdAttrNum
			...
			If (Len(pstrTempAttribute) > 0) Then	
				aProdAttr(i-1) = getAttributes_Duplicate(pstrTempAttribute, mblnMPODuplicates_ssMPO, lngCounter_ssMPO)
			Else
				...
				For Each vItem in Request.Form
					If instr(1, vItem, pstrFieldName) > 0 Then
						pstrTempAttribute = getAttributes_Duplicate(Request.Form(vItem), mblnMPODuplicates_ssMPO, lngCounter_ssMPO) & Replace(vItem, pstrNewVar & cstrSSTextBasedAttributeHTMLDelimiter, cstrSSTextBasedAttributeDelimiter)
...
Function AttributeImageName_new(ByRef aryAttribute, ByVal strFormName)

Dim pstrTemp

	pstrTemp = cstrBaseImagePath & Trim(aryAttribute(enAttributeDetail_ID) & "") + cstrImageSuffix
	If Len(cstrattrdtImage_Field) > 0 Then
		If Len(aryAttribute(enAttribute_Image) & "") > 0 Then
			pstrTemp = aryAttribute(enAttribute_Image)
		End If
	End If
	
	AttributeImageName_new = pstrTemp
					
End Function	'AttributeImageName_new
		...
		Select Case pbytattrDisplayStyle
			...
			Case enAttrDisplay_AttrAsQtyBox:	'attribute as quanity box - assumes MPO installed
					strOut = strOut & "<tr><td>" & pstrAttrTitle & "</td>" _
									...
									& " <input type=text name=""QUANTITY"" size=""4"" onchange=""return validateQty(this, '" & getProductInfo(txtProdId, enProduct_prodMinQty) & "');"">" _
									& "</td></tr>" & vbcrlf
...
Dim pdblAttrDelta
	...
	If isArray(paryAttributes) Then
	For i = 0 To UBound(paryAttributes)
		pobjStringBuilder.Append getAttributeDisplayText(strFormName, i, paryAttributes(i))
	Next 'i
...
Function getAttributeDisplayText(byVal strFormName, byVal i, byRef aryAttribute)
...
Dim pdblAttrDelta
	...
	attrName = aryAttribute(enAttribute_Name)
	pstrattrName = "attr" & CStr(i + 1)
	If Len(CStr(aryAttribute(enAttribute_DisplayStyle))) = 0 Then aryAttribute(enAttribute_DisplayStyle) = enAttrDisplay_Select
	pbytattrDisplayStyle = CLng(aryAttribute(enAttribute_DisplayStyle))

	'Open the attribute display
	Select Case pbytattrDisplayStyle
		Case enAttrDisplay_Select, enAttrDisplay_SelectShowPrice
			pobjStringBuilder.Append "<tr><td align=left valign=top>" _
										& attributeLink_new(aryAttribute, attrName, True) _
	...
	For j = 0 To UBound(aryAttribute(enAttribute_DetailArray))
		Select Case aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Type)
			Case 1
				pdblAttrDelta = CDbl(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Price))
				pstrAttrDisplayAlt = " " & FormatCurrency(CDbl(pdblBasePrice) + CDbl(pdblAttrDelta))
				pstrAttrDisplay = " (Add " & FormatCurrency(pdblAttrDelta) & ")"
			Case 2
				pdblAttrDelta = -1 * CDbl(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Price))
				pstrAttrDisplayAlt = " " & FormatCurrency(CDbl(pdblBasePrice) + pdblAttrDelta)
				pstrAttrDisplay = " (Subtract " & FormatCurrency(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Price)) & ")"
			Case Else
				pdblAttrDelta = 0
				pstrAttrDisplayAlt = " " & FormatCurrency(pdblBasePrice)
				pstrAttrDisplay = ""
		...
		If Len(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Default) & "") = 0 Then
			pblnDefault = CBool(Trim(attrName) <> Trim(attrNamePrev))
		Else
			pblnDefault = CBool(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Default))
		End If
		If pblnDefault Then
			pstrSelectedStyle = " style='background-color:red;'"
			pstrSelectedStyle = " style='background-color:lightgrey;border-left-width:1px; border-right-width:1px; border-top-style:dotted; border-top-width:1px; border-bottom-style:dotted; border-bottom-width:1px'"
		...
		pstrAttrTitle = aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Name)
		pstrFieldName = pstrattrName & cstrSSTextBasedAttributeHTMLDelimiter & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID)

		'Write out the actual attribute detail
		Select Case pbytattrDisplayStyle
			Case enAttrDisplay_Select: 'select
					If pblnChecked Then
						pobjStringBuilder.Append "<option value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """>" & pstrAttrTitle & strAttrPrice & "</option>" & vbcrlf
					Else
						pobjStringBuilder.Append "<option value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """ selected>" & pstrAttrTitle & strAttrPrice & "</option>" & vbcrlf
						pblnChecked = True
					End If
			Case enAttrDisplay_Radio, enAttrDisplay_RadioChangePrice:	'radio
					If pbytattrDisplayStyle = enAttrDisplay_RadioChangePrice Then
					...
					If pblnChecked Then
						pobjStringBuilder.Append "<div " & pstrSelectedStyle & "><input type='radio' name='" & pstrattrName & "' " & pstrOnChangeText & " value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """>" & attributeLink_new(aryAttribute(enAttribute_DetailArray)(j), pstrAttrTitle, False) & strAttrPrice & "</div>" & vbcrlf
					Else
						pobjStringBuilder.Append "<div " & pstrSelectedStyle & "><input type='radio' name='" & pstrattrName & "' " & pstrOnChangeText & " value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """ checked>" & attributeLink_new(aryAttribute(enAttribute_DetailArray)(j), pstrAttrTitle, False) & strAttrPrice & "</div>" & vbcrlf
						pblnChecked = True
					End If
			Case enAttrDisplay_RadioAttributePrice:	'radio
					pstrOptionTemplate = "<div {SelectedStyle}>" _
					...
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{radioName}", pstrattrName)
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{radioID}", pstrattrName & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID))
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{OnChangeText}", pstrOnChangeText)
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{attrdtId}", aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID))
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{AttrTitle}", pstrAttrTitle)
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{attributeLink}", attributeLink_new(aryAttribute(enAttribute_DetailArray)(j), pstrAttrTitle, False))
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{AttrPrice}", FormatCurrency(pdblAttrDelta, 2))
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{attrdtExtra}", FormatCurrency(aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_Extra), 2))
					pstrOptionLineOut = Replace(pstrOptionLineOut, "{Counter}", iCounter)
					...
					If pblnChecked Then
						'pobjStringBuilder.Append "<div " & pstrSelectedStyle & "><input type='radio' name='" & pstrattrName & "' " & pstrOnChangeText & " value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """>" & attributeLink(rsProdAttributes, pstrAttrTitle, False) & strAttrPrice & "</div>" & vbcrlf
						pobjStringBuilder.Append Replace(pstrOptionLineOut, "{checked}", "")
					Else
						'pobjStringBuilder.Append "<div " & pstrSelectedStyle & "><input type='radio' name='" & pstrattrName & "' " & pstrOnChangeText & " value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """ checked>" & attributeLink(rsProdAttributes, pstrAttrTitle, False) & strAttrPrice & "</div>" & vbcrlf
			...
			Case enAttrDisplay_SelectShowPrice:	'select (show price)
					If pblnChecked Then
						pobjStringBuilder.Append "<option value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """>" & pstrAttrTitle & pstrAttrDisplayAlt & "</option>" & vbcrlf
					Else
						pobjStringBuilder.Append "<option value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """ selected>" & pstrAttrTitle & pstrAttrDisplayAlt & "</option>" & vbcrlf
						pblnChecked = True
					End If
			Case enAttrDisplay_SelectChangeImage:	'custom example
					If pblnChecked Then
					...
					Else
						...
						pblnChecked = True
					End If
					mstrssAttributeExtenderjsOut = mstrssAttributeExtenderjsOut & "maryAttrImages['" & EscapeFormName(strFormName) & "'][" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & "] = " & Chr(34) & AttributeImageName_new(aryAttribute(enAttribute_DetailArray)(j), EscapeFormName(strFormName)) & Chr(34) & ";" & vbcrlf
			Case enAttrDisplay_AttrAsQtyBox:	'attribute as quanity box - assumes MPO installed
					pobjStringBuilder.Append "<tr><td>" & pstrAttrTitle & "</td>" _
									& "<td>" _
									& " <input type=hidden name='" & pstrattrName & "' value='" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & "'>" _
									& " <input type=text name=""QUANTITY"" size=""4"" onchange=""return validateQty(this, '" & getProductInfo(txtProdId, enProduct_prodMinQty) & "');"">" _
									& "</td></tr>" & vbcrlf
			Case enAttrDisplay_Custom:	'custom example
					pobjStringBuilder.Append "<input type='text' id='" & pstrFieldName & "' name='" & pstrFieldName & "' title=" & Chr(34) & Server.HTMLEncode(pstrAttrTitle) & Chr(34) & " value=''>" & vbcrlf
			...
			Case Else: 
					...
					Else
						...
						pblnChecked = True
					End If
					'pobjStringBuilder.Append "<option value=""" & aryAttribute(enAttribute_DetailArray)(j)(enAttributeDetail_ID) & """>" & pstrAttrTitle & strAttrPrice & "</option>"
		End Select
	...
	getAttributeDisplayText = pobjStringBuilder.concat
	Set pobjStringBuilder = Nothing

End Function	'getAttributeDisplayText
