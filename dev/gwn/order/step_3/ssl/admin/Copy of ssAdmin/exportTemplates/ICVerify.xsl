...
<xsl:import href="supportingTemplates/date.xsl"/>
...
zip
billing address
tax amount = 0.00 if none
freight amount = 0.00 if none
order date
Invoice #
CW Indicator = 1
cw2/cvc2/cid 
...
<xsl:template match="order" >
	<xsl:if test="string-length(payCardType) > 0">
		<!-- Transaction -->"C1"<xsl:value-of select="$outputDelimeter"/>
		<!-- Clerk -->"jandd.com"<xsl:value-of select="$outputDelimeter"/>
		<!-- Account# (Card Number) -->"<xsl:value-of select="payCardNumber" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- Expiration Month -->"<xsl:value-of select="substring(payCardExpires, 1, 2)" /><xsl:value-of select="substring(payCardExpires, 6, 2)" />"<xsl:value-of select="$outputDelimeter"/>
		...
		<!-- ZIP Code -->
			...
			<xsl:text>"</xsl:text><xsl:value-of select="substring($ZIPCode, 1, 10)" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- Address1 -->"<xsl:apply-templates select="billingAddress/Addr1" mode="escape-CSV" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- tax -->"<xsl:value-of select="orderSTax" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- shipping -->"<xsl:value-of select="orderShippingAmount" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- order date -->"<xsl:value-of select="date:formatdate(shortOrderDate, 'MMM dd, yyyy')" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- Invoice # -->"<xsl:value-of select="orderID" />"<xsl:value-of select="$outputDelimeter"/>
		<!-- CW Indicator -->"1"<xsl:value-of select="$outputDelimeter"/>
		<!-- CVV2 Number -->"<xsl:value-of select="payCardCCV" /><xsl:text>"</xsl:text>
		<!-- End of Line --><xsl:value-of select="$outputDelimeter_CR"/>
	</xsl:if>
