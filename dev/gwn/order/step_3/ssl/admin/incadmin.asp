...
'@ENDVERSIONINFO
	...
	Sub setUpdateAdminPayment(sPaymentType,sPaymentServerPath,sPaymentLogin,sPaymentPassword,sMerchantType,bCardEncode,bCreditCard,bECheck,bCOD,sCODAmount,bPhoneFaxRecorded,bPhoneFaxNon_Recorded,bPO,bPP) 'JF 9/25/01
		...
		Set rsTransMthd = CreateObject("ADODB.RecordSet")
		...
		Set rsTransTypes = CreateObject("ADODB.RecordSet")
			...
			Do While NOT rsTransTypes.EOF
				If rsTransTypes.Fields("transType")	= "Credit Card" Then
					...
					End If	
				ElseIf rsTransTypes.Fields("transType")	= cstrECheckTerm Then
					If bECheck = 1 Then
						rsTransTypes.Fields("transIsActive") = 1
					Else
						rsTransTypes.Fields("transIsActive") = 0	
					End If	
				ElseIf rsTransTypes.Fields("transType")	= cstrCODTerm Then
					If bCOD = 1 Then
						rsTransTypes.Fields("transIsActive") = 1
					Else
						rsTransTypes.Fields("transIsActive") = 0	
					End If	
				ElseIf rsTransTypes.Fields("transType")	= cstrPOTerm Then
					If bPO = 1 Then
						rsTransTypes.Fields("transIsActive") = 1
					Else 
						rsTransTypes.Fields("transIsActive") = 0	
				...
				ElseIf rsTransTypes.Fields("transType")	= cstrPhoneFaxTerm Then
...
'JF 9/25/01 removed Internet Cash stuff
	...
	Sub setUpdateCC(bVisa,bMC,bDiscover,bAMEX,bDiners,bCarteBlanche,bCreditCard)
		...
		Set rsCC = CreateObject("ADODB.RecordSet")
	...
	Function getPaymentMethods
		...
		sString = ""	
		Set rsGetPay = CreateObject("ADODB.RecordSet")
		...
		If Not (rsGetPay.BOF And rsGetPay.EOF) Then
			...
			For iRow = 0 to UBound(aRows, 2)
				If Trim(aRows(0,iRow)) = cstrPhoneFaxTerm Then
	...
	Sub setNewValShip(sNewTotal,sNewShip)
		...
		Set rsValShip = CreateObject("ADODB.RecordSet")
	...
	Sub setUpdateShipping(sShipID)
		...
		Set rsShip = CreateObject("ADODB.RecordSet")
	...
	Function getCategoryList(iValue)
		...
		Set rsCategoryList = CreateObject("ADODB.RecordSet")
	...
	' Object Creation and Query
	Set rsManufacturersList = CreateObject("ADODB.RecordSet")
	...
	' Object Creation and Query
	Set rsVendorList = CreateObject("ADODB.RecordSet")
	...
	Sub setCtryTax(sCtryAbbr,dCtryTax)
		...
		Set rsCtry = CreateObject("ADODB.RecordSet")
	...
	Function findRecord(sCategoryName)
		...
		Set rsCategorySettings = CreateObject("ADODB.recordSet")
	...
	Function findMfgRecord(sMfgName)
		...
		Set rsMfgSettings = CreateObject("ADODB.recordSet")
	...
	Function findVendRecord(sVendName)
		...
		Set rsVendSettings = CreateObject("ADODB.recordSet")
	...
	Function getCtryStates(iType)
		...
		Set rsCS = CreateObject("ADODB.RecordSet")
	...
	Function getTransactionMethods
		...
		Set rsTransMethod = CreateObject("ADODB.RecordSet")
