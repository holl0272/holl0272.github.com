...
<xsl:import href="../supportingTemplates/storeHeader.xsl"/>
...
<xsl:key name="keyShipToStates" match="shippingAddress" use="State"/>
  ...
  <xsl:template match="/">
	...
	<xsl:call-template name="productKeysInReportListing"><xsl:with-param name="productKeys" select="$productKeys" /></xsl:call-template><hr />
	...
	<xsl:template name="taxCollectedByState">
		...
		<xsl:choose>
			<xsl:when test="$collectionMode = '0'"><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/State)" /></xsl:when>
			<xsl:when test="$collectionMode = '1'"><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/Local)" /></xsl:when>
			<xsl:when test="$collectionMode = '2'"><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/StateOnly)" /></xsl:when>
			<xsl:when test="$collectionMode = '3'"><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/LocalOnly)" /></xsl:when>
			<xsl:when test="$collectionMode = '4'"><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/StateAndLocalOnly)" /></xsl:when>
			<xsl:otherwise><xsl:value-of select="sum(msxsl:node-set($salesTaxNodes)/StateAndLocal)" /></xsl:otherwise>
		</xsl:choose>
