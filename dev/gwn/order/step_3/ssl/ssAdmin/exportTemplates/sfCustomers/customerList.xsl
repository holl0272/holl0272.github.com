...
<xsl:template match="item" >
	<xsl:if test="string-length(custEmail) > 0">
		<!-- custLastName --><xsl:value-of select="custLastName"/><xsl:value-of select="$outputDelimeter"/>
		<!-- custFirstName --><xsl:value-of select="custFirstName"/><xsl:value-of select="$outputDelimeter"/>
