...
Dim i
	...
	ReDim paryCustomValues(7)
	
	pstrTableName = "content"
	pstrDisplayField = "contentMetaCustom1"
	pstrTableSortField = "contentMetaCustom1"
	pstrTableSortOrder = "Asc"
	
	'Note: contentContentType is hardcoded to 9 to set filter to automatically display only page fragment types
	plngFixedContentType = 9
	...
	paryCustomValues(0) = Array("ID", "contentID", "", enDisplayType_hiddenDisplay, "4", "", enDatatype_number, True, "", "")
	paryCustomValues(1) = Array("Page", "contentMetaCustom1", "", enDisplayType_textbox, "50", "", enDatatype_string, True, "", "")
	paryCustomValues(2) = Array("Content Type", "contentContentType", plngFixedContentType, enDisplayType_hidden, "4", "", enDatatype_number, True, "", "")
	paryCustomValues(3) = Array("Content", "contentContent", "", enDisplayType_textarea_WithHTMLSelect, "10", "", enDatatype_string, True, "", "Optional HTML fragment to display at the top of the custom manufacturer page. If left blank the manufacturer name will be used.")
	paryCustomValues(4) = Array("Display", "contentApprovedForDisplay", "", enDisplayType_checkbox, "20", "", enDatatype_number, True, "", "")
	paryCustomValues(5) = Array("Notes", "contentMetaCustom2", "", enDisplayType_textarea, "2", "", enDatatype_string, False, "", "")
	paryCustomValues(6) = Array("Date Created", "contentDateCreated", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, False, "", "")
	paryCustomValues(7) = Array("Date Modified", "contentDateModified", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, False, "", "")
...
'On Error Resume Next
    ...
    If Len(strErrorMessage) = 0 Then
		...
		objRS.Update
		Application.Contents.Remove("cmsFragment_" & paryCustomValues(1)(enCustomField_FieldValue))
...
'**********************************************************
	...
	mstrPageTitle = "Sandshot Content Management System - Page Content Administration"
...
If mstrAction = "FixMissingFragment" Then
	Response.Write "DisplayMainSection('itemDetail');" & vbcrlf
	Response.Write "btnNewItem_onclick(document.getElementById('btnNewItem'));" & vbcrlf
	Response.Write "document.getElementById('contentMetaCustom1').value = '" & LoadRequestValue("ViewID") & "';" & vbcrlf
	Response.Write "alert('You have been directed to this page because the content for {" & LoadRequestValue("ViewID") & "} is missing.\n\nPlease fill in the content section and press the save button.');" & vbcrlf
	Response.Write "document.getElementById('contentContent').focus();" & vbcrlf
End If
%>
}
...
<div style="border: solid 1pt black;padding: 5pt 5pt 5pt 5pt;">
<strong><u>Instructions for use:</u></strong><br />
Content will be written out exactly as entered in the <em>Content</em> section.<br />
<em>ID</em> is a key for pages which call getPageFragmentByID. Otherwise, it is for reference only.<br />
<em>Page</em> is a key for pages which call getPageFragmentByKey. Otherwise, it is for reference only.<br />
<em>Notes</em> is for reference only.<br />
For the content to display you must include one of the following code fragments on the desired page:<br />
<code>Response.Write getPageFragmentByID({ID})</code>
<br />
<code>Response.Write getPageFragmentByKey({Page})</code>
</div>
...
Sub WriteFormOpener
%>
<form id="frmData" name="frmData" onsubmit="return ValidInput(this);" method="post" action="ssCMS_PageFragmentAdmin.asp">
	...
	Select Case ary(lngPos)(enCustomField_FieldName)
		Case "contentAuthorID":
			...
			If len(ary(lngPos)(enCustomField_FieldValue)) > 0 then
				Response.Write " - "
				pstrSQL = "SELECT custLastName & "", "" & custFirstName As customerName FROM sfCustomers Where custID=" & ary(lngPos)(enCustomField_FieldValue)
				pstrSQL = "SELECT custLastName As customerName FROM sfCustomers Where custID=" & ary(lngPos)(enCustomField_FieldValue)
				Set pobjRS = GetRS(pstrSQL)
				If pobjRS.EOF Then
					Response.Write "No customer exists for ID " & ary(lngPos)(enCustomField_FieldValue)
				Else
					Response.Write "<a href='sfCustomerAdmin.asp?viewID=" & ary(lngPos)(enCustomField_FieldValue) & "' title=""View Customer Record"">" & Trim(pobjRS.Fields("customerName").Value & "") & "</a>"
				End If
				Call ReleaseObject(pobjRS)
			Else
		...
		Case "contentReferenceID":
			...
			If len(ary(lngPos)(enCustomField_FieldValue)) > 0 then
				Response.Write " - "
				pstrSQL = "Select prodID, prodName From sfProducts Where sfProductID=" & ary(lngPos)(enCustomField_FieldValue)
				Set pobjRS = GetRS(pstrSQL)
				If pobjRS.EOF Then
					Response.Write "No Product Found Matching productID = " & ary(lngPos)(enCustomField_FieldValue)
				Else
					Response.Write "<a href='sfProductAdmin.asp?Action=ViewProduct&ViewID=" & Trim(pobjRS.Fields("prodID").Value & "") & "' title=""View product detail"">" & Trim(pobjRS.Fields("prodName").Value & "") & "</a>"
				End If
				Call ReleaseObject(pobjRS)
			Else
				Response.Write "No Product defined for this review"
