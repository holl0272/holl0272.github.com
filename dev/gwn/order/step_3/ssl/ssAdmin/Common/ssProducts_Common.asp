...
Dim pstrProductName
	...
	If cblnSF5AE Then
		...
		With pobjRSProductCategory
			...
			If .EOF Then
				Call getProductInfo(strProductID, pstrProductName)
				pstrSQL = "Insert Into sfSubCatDetail (ProdID,ProdName,subcatCategoryId) Values (" & wrapSQLValue(strProductID, False, enDatatype_string) & ", " & wrapSQLValue(pstrProductName, False, enDatatype_string) & ", " & wrapSQLValue(lngCategoryID, False, enDatatype_number) & ")"
				cnn.Execute pstrSQL,,128
				pblnAdded = True
			End If
			.Close
...
Dim pstrTempCategory
	...
	Else
		pstrSQL = "Select catID as uid, catName" _
				& " From sfCategories" _
				& " Where catName=" & wrapSQLValue(strCategoryName, False, enDatatype_string)
	End If
...
Dim plngSubCatID
	...
	pstrSQL = "INSERT INTO sfSub_Categories (subcatCategoryId, subcatName, Depth, HasProds, bottom,  subcatIsActive, CatHierarchy)" _
			& " VALUES (" _
	...
	If Not pobjRS.EOF Then
		plngSubCatID = pobjRS.Fields("subcatID").Value
		pstrSQL = "Update sfSub_Categories Set CatHierarchy=" & wrapSQLValue("none-" & plngSubCatID, False, enDatatype_string) & " Where subCatID=" & wrapSQLValue(plngSubCatID, False, enDatatype_number)
		'debugprint "pstrSQL", pstrSQL
		cnn.Execute pstrSQL,,128
	Else
		plngSubCatID = -1
	...
	CreatePrimarySubCategory = plngSubCatID
...
%>
<!--#include file="ssProducts_Custom.asp"-->
