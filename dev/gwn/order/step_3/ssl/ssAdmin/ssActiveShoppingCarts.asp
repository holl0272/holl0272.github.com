...
Dim i
	...
	pstrKeyFieldName = "odrdttmpID"
	pstrKeyFieldIsNumeric = True
	pstrExportTemplateDirectory = "-"	'Note: this must end in a /, use - to not display export capability
	pstrDisplayField = "odrdttmpID"
	pstrTableSortField = "visitorLastVisited"
	pstrTableSortOrder = "Asc"
	pstrPageTitle = "Active Shopping Carts Administration"
	pstrPageVersion = "2.00.004"
	pstrURL = "ssActiveShoppingCarts.asp"
	...
	ReDim paryCustomValues(1)
	paryCustomValues(0) = Array("odrdttmpID", "odrdttmpID", "", enDisplayType_hidden, "4", "", enDatatype_number, False, "", "Customer ID, hidden field")
	paryCustomValues(1) = Array("Last Access", "visitorLastVisited", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, True, "")
...
'        .CursorType = 1 'adOpenKeySet	- Have to use KeySet for SQL Server
        pstrSQL = "SELECT visitors.visitorDateCreated, visitors.visitorLastVisited, visitors.visitorID, visitors.visitorLastSearch, visitors.visitorRecentlyViewedProducts, visitors.vistor_HTTP_REFERER, visitors.visitorOrderID, sfTmpOrderDetails.odrdttmpQuantity, sfProducts.prodID, sfProducts.prodName, sfCustomers.custLastName, sfCustomers.custFirstName, sfCustomers.custEmail" _
				& " FROM (visitors LEFT JOIN sfCustomers ON visitors.visitorCustomerID = sfCustomers.custID) LEFT JOIN (sfTmpOrderDetails LEFT JOIN sfProducts ON sfTmpOrderDetails.odrdttmpProductID = sfProducts.prodID) ON visitors.visitorID = sfTmpOrderDetails.odrdttmpSessionID" _
				& " ORDER BY visitors.visitorLastVisited DESC , visitors.visitorID, sfProducts.prodName"
        pstrSQL = "SELECT visitors.visitorDateCreated, visitors.visitorLastVisited, visitors.visitorID, visitors.visitorLastSearch, visitors.visitorRecentlyViewedProducts, visitors.vistor_HTTP_REFERER, visitors.visitorOrderID, sfTmpOrderDetails.odrdttmpQuantity, sfProducts.prodID, sfProducts.prodName, sfAttributes.attrName, sfAttributeDetail.attrdtName, sfTmpOrderAttributes.odrattrtmpAttrText, sfCustomers.custLastName, sfCustomers.custFirstName, sfCustomers.custEmail" _
				& " FROM ((visitors LEFT JOIN sfCustomers ON visitors.visitorCustomerID = sfCustomers.custID) LEFT JOIN ((sfTmpOrderDetails LEFT JOIN sfTmpOrderAttributes ON sfTmpOrderDetails.odrdttmpID = sfTmpOrderAttributes.odrattrtmpOrderDetailId) LEFT JOIN sfProducts ON sfTmpOrderDetails.odrdttmpProductID = sfProducts.prodID) ON visitors.visitorID = sfTmpOrderDetails.odrdttmpSessionID) LEFT JOIN (sfAttributes RIGHT JOIN sfAttributeDetail ON sfAttributes.attrID = sfAttributeDetail.attrdtAttributeId) ON sfTmpOrderAttributes.odrattrtmpAttrID = sfAttributeDetail.attrdtID" _
				& " ORDER BY visitors.visitorLastVisited DESC , visitors.visitorID, sfProducts.prodName"
        pstrSQL = "SELECT visitors.visitorDateCreated, visitors.visitorLastVisited, visitors.visitorID, visitors.visitorLastSearch, visitors.visitorRecentlyViewedProducts, visitors.vistor_HTTP_REFERER, visitors.visitorOrderID, sfTmpOrderDetails.odrdttmpQuantity, sfProducts.prodID, sfProducts.prodName, sfTmpOrderDetails.odrdttmpID, sfTmpOrderAttributes.odrattrtmpID, sfAttributes.attrName, sfAttributeDetail.attrdtName, sfTmpOrderAttributes.odrattrtmpAttrText, sfCustomers.custLastName, sfCustomers.custFirstName, sfCustomers.custEmail" _
				& " FROM ((visitors LEFT JOIN sfCustomers ON visitors.visitorCustomerID = sfCustomers.custID) LEFT JOIN (sfTmpOrderDetails LEFT JOIN sfProducts ON sfTmpOrderDetails.odrdttmpProductID = sfProducts.prodID) ON visitors.visitorID = sfTmpOrderDetails.odrdttmpSessionID) LEFT JOIN (sfTmpOrderAttributes LEFT JOIN (sfAttributes RIGHT JOIN sfAttributeDetail ON sfAttributes.attrID = sfAttributeDetail.attrdtAttributeId) ON sfTmpOrderAttributes.odrattrtmpAttrID = sfAttributeDetail.attrdtID) ON sfTmpOrderDetails.odrdttmpID = sfTmpOrderAttributes.odrattrtmpOrderDetailId" _
				& " ORDER BY visitors.visitorLastVisited DESC , visitors.visitorID, sfProducts.prodName, sfTmpOrderAttributes.odrattrtmpID"
...
<table id="tblMainPage" class="tbl" style="border-collapse: collapse" bordercolor="#111111" width="100%" cellpadding="1" cellspacing="0" border="0" rules="none">
  ...
  <tr>
	...
	<%
		...
		Dim pobjTmpOrders
		Set pobjTmpOrders = mclsItem.rsItems
	%>
	<table id='tblSummary' class="tbl" style="border-collapse: collapse" bordercolor="#111111" width="100%" cellpadding="1" cellspacing="0" border="1" bgcolor="whitesmoke">
	  <tr class='tblhdr'>
		<th>ID</th>
		<th>Last Visit</th>
		<th>Last Search</th>
	  ...
	  If pobjTmpOrders.RecordCount > 0 Then
		pobjTmpOrders.moveFirst
		Do While Not pobjTmpOrders.EOF
			mblnNewVisitor = CBool(pobjTmpOrders.Fields("visitorID").Value <> PrevVisitorID)
			If mblnNewVisitor Then
				PrevVisitorID = pobjTmpOrders.Fields("visitorID").Value
				mblnEvenProduct = Not mblnEvenProduct
		%>
		<tr <%
				If getCookie_SessionID = Trim(PrevVisitorID) Then
					Response.Write " class='Selected'"
				ElseIf Not mblnEvenProduct Then
					Response.Write " class='Inactive'"
				End If
			%>>
			<td><%= PrevVisitorID %></td>
			<td><%= pobjTmpOrders.Fields("visitorLastVisited").Value %></td>
			<td><%= pobjTmpOrders.Fields("visitorLastSearch").Value %></td>
			<td><%= pobjTmpOrders.Fields("visitorRecentlyViewedProducts").Value %></td>
			<td><%= pobjTmpOrders.Fields("visitorOrderID").Value %></td>
			...
			<td><%= pobjTmpOrders.Fields("vistor_HTTP_REFERER").Value %></td>
		</tr>
		<%
			End If	'mblnNewVisitor
			
			If Not isNull(pobjTmpOrders.Fields("odrdttmpQuantity").Value) Then
				If PrevOdrdttmpID <> pobjTmpOrders.Fields("odrdttmpID").Value Then
					PrevOdrdttmpID = pobjTmpOrders.Fields("odrdttmpID").Value
				%>
				<tr <% If Not mblnEvenProduct Then Response.Write " class='Inactive'" %>>
					<td>&nbsp;</td>
					<td colspan="7">- (<%= pobjTmpOrders.Fields("odrdttmpQuantity").Value %>) <%= pobjTmpOrders.Fields("prodID").Value %>: <%= pobjTmpOrders.Fields("prodName").Value %></td>
				</tr>
				<%
				End If
				If Not isNull(pobjTmpOrders.Fields("odrattrtmpID").Value) Then
				%>
				<tr <% If Not mblnEvenProduct Then Response.Write " class='Inactive'" %>>
					<td>&nbsp;</td>
					<td colspan="7">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%= pobjTmpOrders.Fields("attrName").Value %>: <%= pobjTmpOrders.Fields("attrdtName").Value %></td>
				</tr>
				<%
				End If
			End If
