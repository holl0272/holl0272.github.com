...
Dim menuIndex
	...
	pstrPageTitle = "Sandshot Content Management System - Design Administration"
	pstrVersion = "1.00.001b"
	
	plngFixedContentType = enContentType_Designs	'hardcoded to set filter to automatically display only Design types
	
	pstrTableName = "content"
	pstrDisplayField = "contentTitle"
	pstrTableSortField = "contentTitle"
	...
	ReDim paryCustomValues(17)
	menuIndex = 0
	paryCustomValues(menuIndex) = Array("contentID", "contentID", "", enDisplayType_hiddenDisplay, "4", "", enDatatype_number, False, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Display Name", "contentTitle", "", enDisplayType_textbox, "50", "", enDatatype_string, True, "", "Anchor text to use for links in. If left blank the manufacturer name will be used."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Approved For Display", "contentApprovedForDisplay", "", enDisplayType_checkbox, "20", "", enDatatype_number, True, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Theme", "contentReferenceID",   "", enDisplayType_select, "5", "Select contentID, contentTitle FROM content Where contentContentType=" & enContentType_Themes & " Order By contentTitle", enDatatype_number, True, ""):	menuIndex = menuIndex + 1

	paryCustomValues(menuIndex) = Array("Template Page", "contentTemplatePage", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "This is the template page which will be used in place of the default page."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("URL", "contentPageName", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "The url to display in the site map. If this is left blank the title will be used as in title.htm"):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Page Content", "contentContent", "", enDisplayType_textbox_WithHTMLSelect, "4", "", enDatatype_string, False, "", "Optional HTML fragment to display at the top of the custom manufacturer page. If left blank the manufacturer name will be used."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Image", "contentContentFilePath", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "Optional image."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("META - Title", "contentPageTitle", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "Title to use for the page and for the link. If left blank the manufacturer name will be used."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("META - Description", "contentMetaDescription", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "Description which will appear for the description meta tag. This should be kept fairly brief. If left blank the manufacturer name will be used."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("META - Keywords", "contentMetaKeywords", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "The keywords meta tag. This should be kept fairly brief. If left blank the manufacturer name will be used."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("META - Author", "contentMetaAuthor", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", "The author meta tag. This should be kept fairly brief."):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Custom 1", "contentMetaCustom1", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Custom 2", "contentMetaCustom2", "", enDisplayType_textbox, "50", "", enDatatype_string, False, "", ""):	menuIndex = menuIndex + 1

	paryCustomValues(menuIndex) = Array("Date Created", "contentDateCreated", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, False, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Date Modified", "contentDateModified", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, False, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Sort Order", "contentSortOrder", "", enDisplayType_textbox, "4", "", enDatatype_number, True, "", ""):	menuIndex = menuIndex + 1
	paryCustomValues(menuIndex) = Array("Content Type", "contentContentType", plngFixedContentType, enDisplayType_hidden, "4", "", enDatatype_number, True, "", ""):	menuIndex = menuIndex + 1
...
'On Error Resume Next
	...
	pstrSQL = "Delete From contentCategoryAssignments Where contentCategoryAssignmentContentID=" & wrapSQLValue(vntID, False, paryCustomValues(0)(enCustomField_DataType))
	cnn.Execute pstrSQL, , 128
...
'On Error Resume Next
    ...
    If Len(strErrorMessage) = 0 Then
		...
		objRS.Update
		pvntID = objRS.Fields(paryCustomValues(0)(enCustomField_FieldName)).Value
...
Private Sub UpdateCustomOptions()
...
Dim pstrSQL
	...
	pstrSQL = "Select contentCategoryAssignmentID, contentCategoryAssignmentCategoryID From contentCategoryAssignments Where contentCategoryAssignmentContentID=" & pvntID & " Order By contentCategoryAssignmentCategoryID Asc"
	Set pobjRSSelections = GetRS(pstrSQL)
	For i = 0 To UBound(paryOptions)
		pstrOptions = Trim(paryOptions(i))
		pobjRSSelections.Filter = "contentCategoryAssignmentCategoryID=" & pstrOptions
		If pobjRSSelections.EOF Then
			pstrSQL = "Insert Into contentCategoryAssignments (contentCategoryAssignmentContentID, contentCategoryAssignmentCategoryID) Values (" & pvntID & "," & pstrOptions & ")"
			cnn.Execute pstrSQL,,128
		End If
	Next 'i
	...
	Do While Not pobjRSSelections.EOF
		...
		For i = 0 To UBound(paryOptions)
			pstrOptions = Trim(paryOptions(i))
			If CStr(pstrOptions) = CStr(pobjRSSelections.Fields("contentCategoryAssignmentCategoryID").Value) Then
				pblnDeletedOption = False
				Exit For
			End If
		Next 'i
		If pblnDeletedOption Then
			pstrSQL = "Delete From contentCategoryAssignments Where contentCategoryAssignmentID=" & pobjRSSelections.Fields("contentCategoryAssignmentID").Value
			cnn.Execute pstrSQL,,128
		End If
...
<!--#include file="../SFLib/ssmodCMS.asp"-->
...
var theForm = theButton.form;
    ...
    document.getElementById("divKeyID").innerHTML = "{id}";
    theForm.contentTemplatePage.value = "?cmsID={id}";
    theForm.contentContentType.value = <%= .FixedContentType %>;
    //document.getElementById("divKeyID").style.display = "none";
}
...
End If
%>
    theForm.contentContentType.selectedIndex = 0;
...
var  strSection = frmData.Show.value;
	...
	/*
    if (theForm.contentReferenceID.selectedIndex == -1)
    {
		alert('Please select a manufacturer');
		theForm.contentReferenceID.select();
		theForm.contentReferenceID.focus();
		return false;
    }
	*/
...
'If Len(mclsItem.Message) > 0 Then Response.Write "<p><a href=""../../ssDebuggingConsole.asp?reset_sfManufacturers=1"">Apply Changes To Site</a></p>"
%>
<table id="tblMainPage" class="tbl" style="border-collapse: collapse" bordercolor="#111111" width="100%" cellpadding="1" cellspacing="0" border="0" rules="none">
  <tr>
	...
	<th width="90%" align=right><span class="pagetitle2"><%= mstrPageTitle %></span>&nbsp;<input class="butn" type=button value="?" onclick="OpenHelp('ssHelpFiles/ContentManagement/help_ContentManagement.htm')" id="btnHelp" name="btnHelp" title="Release Version <%= mstrssAddonVersion %>"></th>
  </tr>
  <tr>
...
Sub WriteFormOpener
%>
<form id="frmData" name="frmData" onsubmit="return ValidInput(this);" method="post" action="ssCMS_UserType1Admin.asp">
...
<table class="tbl" width="100%" cellpadding="3" cellspacing="0" border="1" rules="none" id="tblCustom">
	...
	%>
      <tr>
        <td class="Label" valign="top"><a href="ssCMS_UserType3Admin.asp">Breeds</a>:</td>
        <td>
		<%
		Dim pobjRSOptions
		Dim pobjRSSelections
		...
		Set pobjRSOptions = GetRS("Select contentID, contentTitle From content Where (contentApprovedForDisplay=1 Or contentApprovedForDisplay=-1) And contentContentType=" & enContentType_Breeds & " Order By contentTitle Asc")
		If Len(mclsItem.ID) > 0 Then Set pobjRSSelections = GetRS("Select contentCategoryAssignmentCategoryID From contentCategoryAssignments Where contentCategoryAssignmentContentID=" & mclsItem.ID & " Order By contentCategoryAssignmentCategoryID Asc")
		i = 0
		Do While Not pobjRSOptions.EOF
				i = i + 1
				
				If isObject(pobjRSSelections) Then
					pobjRSSelections.Filter = "contentCategoryAssignmentCategoryID=" & pobjRSOptions.Fields("contentID").Value
					pblnSelected = CBool(Not pobjRSSelections.EOF)
				Else
					pblnSelected = False
				End If
				pstrHTMLout = "<input  type=""checkbox"" name=""itemOption"" id=""itemOption" & i & """ Value=""" & pobjRSOptions.Fields("contentID").Value & """ " & isChecked(pblnSelected) & "><label for=""itemOption" & i & """>" & Trim(pobjRSOptions.Fields("contentTitle").Value) & "</label><br />"
				Response.Write pstrHTMLout
			pobjRSOptions.MoveNext
		Loop
		Call ReleaseObject(pobjRSOptions)
		Call ReleaseObject(pobjRSSelections)
		%>
        </td>
      </tr>
...
Function CustomDisplayText(byVal lngPos, byRef ary)
	...
	Select Case ary(lngPos)(enCustomField_FieldName)
		...
		Case "contentReferenceID":
			CustomDisplayText = "<a href='ssCMS_UserType2Admin.asp' title='View Pricing Levels'>" & ary(lngPos)(enCustomField_DisplayText) & "</a>"
		Case Else:
...
Dim pbytPricingLevelID
	...
	Select Case ary(lngPos)(enCustomField_FieldName)
		...
		<%
		Case "contentID":
 			Response.Write "<div id=""divKeyID"">"
 			...
 			Response.Write "</div>"
		Case "xcontentContentType":
 			Response.Write writeHTMLFormElement(ary(lngPos)(enCustomField_DisplayType), ary(lngPos)(enCustomField_DisplayLength), ary(lngPos)(enCustomField_FieldName), ary(lngPos)(enCustomField_FieldName), ary(lngPos)(enCustomField_FieldValue), ary(lngPos)(enCustomField_sqlSource), " MakeDirty(this);") 
 			%> <a href="ssCMS_ContentTypeAdmin.asp">Edit</a><%
		Case Else:
			Response.Write writeHTMLFormElement(ary(lngPos)(enCustomField_DisplayType), ary(lngPos)(enCustomField_DisplayLength), ary(lngPos)(enCustomField_FieldName), ary(lngPos)(enCustomField_FieldName), ary(lngPos)(enCustomField_FieldValue), ary(lngPos)(enCustomField_sqlSource), " MakeDirty(this);") 
	End Select
