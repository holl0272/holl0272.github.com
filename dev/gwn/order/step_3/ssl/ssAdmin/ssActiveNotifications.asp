...
'        .CursorType = 1 'adOpenKeySet	- Have to use KeySet for SQL Server
        ...
        pstrSQL = "SELECT notifyMe.notifyMeID, sfProducts.prodID, sfProducts.prodName, sfAttributes.attrName, sfAttributeDetail.attrdtName, notifyMe.notifyEmail, notifyMe.notifyDateCreated, notifyMe.notifyDateNotified, notifyMe.notifyLastName, notifyMe.notifyFirstName" _
				& " FROM ((sfAttributes RIGHT JOIN sfAttributeDetail ON sfAttributes.attrID = sfAttributeDetail.attrdtAttributeId) RIGHT JOIN (notifyMe LEFT JOIN notifyMeAttributes ON notifyMe.notifyMeID = notifyMeAttributes.notifyMeID) ON sfAttributeDetail.attrdtID = notifyMeAttributes.AttrdtID) INNER JOIN sfProducts ON notifyMe.notifyProdID = sfProducts.prodID" _
				& " WHERE notifyMe.notifyType='0'" _
...
'**********************************************************
	...
	mstrPageTitle = "Customer Notifications Administration"
  ...
  <tr>
	...
	<table class='tbl' width='100%' style="border-collapse: collapse" cellpadding='0' cellspacing='0' border='1' bgcolor='whitesmoke' id='tblSummary' rules="none">
	  ...
	  Do While Not pobjTmpOrders.EOF
		mblnNewVisitor = CBool(pobjTmpOrders.Fields("notifyMeID").Value <> PrevnotifyMeID)
		If mblnNewVisitor Then
			PrevnotifyMeID = pobjTmpOrders.Fields("notifyMeID").Value
       ...
       ' pstrSQL = "SELECT notifyMeID, sfCustomers.custEmail, sfCustomers.custLastName, sfCustomers.custFirstName, sfSavedOrderDetails.odrdtsvdQuantity, sfProducts.prodID, sfProducts.prodName, sfAttributes.attrName, sfAttributeDetail.attrdtName, sfSavedOrderAttributes.odrattrsvdAttrText, sfSavedOrderDetails.odrdtsvdDate" _
		%>
		<tr <%= mstrClass %>>
			<td><%= pobjTmpOrders.Fields("prodID").Value %></td>
			<td><%= pobjTmpOrders.Fields("prodName").Value %></td>
			<td><%= pobjTmpOrders.Fields("notifyDateCreated").Value %></td>
			<td><%= pobjTmpOrders.Fields("notifyDateNotified").Value %></td>
			<td><%= pobjTmpOrders.Fields("notifyEmail").Value %></td>
			<td><%= pobjTmpOrders.Fields("notifyLastName").Value %>, <%= pobjTmpOrders.Fields("notifyFirstName").Value %></td>
		</tr>
		<%
		...
		<tr <%= mstrClass %>>
			...
			<td>
			<%
			Response.Write " - " & pobjTmpOrders.Fields("attrName").Value & " " & pobjTmpOrders.Fields("attrdtName").Value & "<br />"
			%>
			</td>
			<td colspan="4">&nbsp;</td>
		</tr>
...
Sub WriteFormOpener
%>
<form id="frmData" name="frmData" onsubmit="return ValidInput(this);" method="post" action="ssActiveNotifications.asp">
