...
Dim i
	...
	pstrTableName = "ssOrderProcessingLog"
	pstrDisplayField = "orderProcessingID"
	pstrTableSortField = "orderProcessingDate"
	...
	paryCustomValues(0) = Array("ID", "orderProcessingID", "", enDisplayType_hidden, "4", "", enDatatype_number, False, "", "ID, hidden field")
	paryCustomValues(1) = Array("Order", "orderProcessingOrderID", "", enDisplayType_textbox, "40", "", enDatatype_string, True, "", "")
	paryCustomValues(2) = Array("Date", "orderProcessingDate", "", enDisplayType_textbox_WithDateSelect, "20", "", enDatatype_date, True, "")
	paryCustomValues(3) = Array("Action", "orderProcessingAction",   "", enDisplayType_textbox, "20", "", enDatatype_string, True, "")
	paryCustomValues(4) = Array("Success", "orderProcessingSuccess", "", enDisplayType_checkbox, "20", "", enDatatype_boolean, True, "", "")
...
'**********************************************************
	...
	mstrPageTitle = "Order Processing Log Administration"
	mstrssAddonVersion = "1.00.001"

	Call CheckLoginStatus(True)

	mlngMaxRecords = LoadRequestValue("PageSize")
	If len(mlngMaxRecords) = 0 Then mlngMaxRecords = 100
...
Sub WriteFormOpener
%>
<form id="frmData" name="frmData" onsubmit="return ValidInput(this);" method="post" action="ssOrderProcessingLogAdmin.asp">
...
Const pstrOrderDetailLink = "<a href='ssOrderAdmin.asp?OrderID={orderID}&Action=ViewOrder&optDisplay=0&optDate_Filter=0&optPayment_Filter=0&optShipment_Filter=0'>{orderID}</a>"
	...
	Select Case ary(lngPos)(enCustomField_FieldName)
		...
		<%
		Case "orderProcessingOrderID":
			Response.Write Replace(pstrOrderDetailLink, "{orderID}", ary(lngPos)(enCustomField_FieldValue))
		Case "orderProcessingDate":
			Response.Write ary(lngPos)(enCustomField_FieldValue)
		Case "orderProcessingAction":
			Dim paryTemp
			If Len(ary(lngPos)(enCustomField_FieldValue)) > 0 Then
				paryTemp = Split(ary(lngPos)(enCustomField_FieldValue), "|")
				Response.Write paryTemp(0)
				For i = 1 To UBound(paryTemp)
					Response.Write "<br />" & paryTemp(i)
				Next 'i
			End If
		Case Else:
